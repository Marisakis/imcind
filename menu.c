...
int initMenu()
{
  ...
  //printf("array initialisation through malloc succes, ready to add first member\n");
  return 0;
}
...
int _checkSize()
{
  ...
  {
    // printf("error: initialise menu first\n");
    return -2;
  }
...
//Args: a non-null char pointer and a function pointer. Passing a null function pointer is effectively the same as addMenuOption()
...
int addMenuOption(const char * menu_option_text)
{
  ...
  else
  {
    ...
    current_number_of_menus ++;
    //printf("member menu option added\n");
  }
  return 0;
}
